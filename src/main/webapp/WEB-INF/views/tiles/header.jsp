...
.alarmCnt{color: #ffffff; background-color: #d30000; width: 20px; font-size: 11px; border-radius: 20px; text-align: center; line-height: normal;}
</style>
		...
		<ul>
			<c:choose>
				<c:when test="${vo.commonDetailVO.comdDesc eq 'ROLE_STUDENT'}">
					<li><a href="/student/mypage">My Portal</a></li>
				</c:when>
				<c:when test="${vo.commonDetailVO.comdDesc eq 'ROLE_PROFESSOR'}">
					<li><a href="/professor/mypage">My Portal</a></li>
				</c:when>
				<c:when test="${vo.commonDetailVO.comdDesc eq 'ROLE_EMPLOYEE'}">
					<li><a href="/employee/mypage">My Portal</a></li>
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${vo.commonDetailVO.comdDesc eq 'ROLE_STUDENT'}">
					<li><a href="/student/enrolment/app">수강신청</a></li>
				</c:when>
				<c:when test="${vo.commonDetailVO.comdDesc eq 'ROLE_PROFESSOR'}">
			...
			<li><a href="/common/myportlet">포틀릿설정</a></li>
		</ul>
	</div>
...
<!-- 	</div> -->
	<div class="header_right_box" style="display:flex; height:100%; align-items:center;">
		...
		<a href="#none" class="alarm alarm_btn" style="position: relative; margin-right:10px;">
			...
			<!-- //////////////////////////////새로만든알람화면 -->
			<div class="alarm_box dropdown-menu" style="padding:20px; position: fixed; top:70px; right:40px; border-radius: 10px; width:350px; height: 600px; background-color: #ffffff; border: 1px solid #f2f3f4;">
				...
				<div style="width:100%; overflow-x: scroll; margin-top: 15px;">
					...
					<a class="clickActive" id="allAlarm"  href="#none">전체<span style="font-size: 11px;" id="cntAll">(5)</span></a>
					<a id="mailAlarm" href="#none">메일</a>
					<a id="academicAlarm" href="#none">강의</a>
					<c:if test="${vo.comdCd != 'USER02'}">
						<a id="recordAlarm" href="#none">학적</a>
					</c:if>
...
<script>
	$(document).ready(function() {
		...
		$('.alarm_btn').click(function() {
			...
			$(".portletlock").toggleClass("removeLock");
		});
