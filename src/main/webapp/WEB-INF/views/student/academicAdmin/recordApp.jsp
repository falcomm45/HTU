...
<div class="record_box">
	...
	<span style="font-size:12px; color:#888888;">></span>
	<span style="font-size:12px; color:#888888;"><a href="/student/academicAdmin/recordApp">휴학신청</a></span>
	</div>
	

<form name="frm"
	action="/student/academicAdmin/recordAppPost?${_csrf.parameterName}=${_csrf.token}"
	method="post" enctype="multipart/form-data" class="record_form" id="capture">
	...
	<input type="text" name="comdCd" id="comdCd" hidden="hidden" value="RECD02"> 
		<div class="table_topic" style="margin-bottom:40px; padding-bottom:15px; border-bottom:1px solid #d1d0d0; display:flex; justify-content:space-between;">
		<span>휴학(연장)신청서</span>
		<button type="button" id="btnAuto" class="btn-two mini gray" style="width: 80px; height:25px;">자동완성</button>
		</div>
	<div class="record_form_title">
	  ...
	  <div style="display:flex; margin-bottom:40px; align-items: center;">
	 	<div style="flex:1; display:flex; align-items: center;">
			...
			<input id="recYear" name="recYear" value="${year}" style="width:100%; font-size:15px; border:1px solid #888888;" readonly/>
		</div>
		<div style="flex:1; display:flex; align-items: center;">
	  ...
	  <div style="display:flex; flex-direction:column; margin-bottom:20px;  margin-top:10px;">
	  	<p style="margin-bottom:10px;">휴학사유</p>
	  	<textarea id="recRsn" name="recRsn" style="height:230px; padding:10px; line-height:20px; font-size:14px;"></textarea>
	  </div>
	</div>
...
<script>
 $('#btnAuto').on("click",function(){
	 $('#recRsn').val("학교 공부만으로는 역량을 부족한 것 같아 8개월정도 대덕인재개발원 전자정부 표준 프레임워크 기반 풀-스택 개발자 양성과정을 수료하고자 합니다.");
 });
</script>
			...
			if (result.isConfirmed) {
				...
				setTimeout(() => {
					...
					Swal.fire({
						...
						// Redirect to recordList page
						window.location.href = "/student/academicAdmin/recordList";
					});
				}, 1000);
			}
		});
