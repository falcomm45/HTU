...
<style>
    /* 학생리스트 css */
    .stuadmin_box {
        width: 100%; padding:40px; background-color:#ffffff; height:100%;
    }
    .stuadmin_box .table_topic {
        font-size: 20px;
        padding-bottom: 5px;
    }
    .stuadmin_box .table_topic i {                           
        margin-left: 10px;
    }
    .stuadmin_box table {
        font-size: 14px;
	...
	.stuadmin_box tr{height:40px!important; border-bottom:1px solid #ebebeb;}
	.stuadmin_box tr:first-child{background-color:#EEF1F6;}
	.stuadmin_box tr:hover{background-color:#f2f3f4;}
	.stuadmin_box tr:first-child:hover{background-color:#EEF1F6;}
	
    .stuadmin_box tr td:nth-child(1) {
        width: 10%;
    }
    .stuadmin_box tr td:nth-child(2) {
        width: 15%;
    }
    .stuadmin_box tr td:nth-child(3) {
        width: 15%;
    }
    .stuadmin_box tr td:nth-child(4) {
        width: 15%;
    }
    .stuadmin_box tr td:nth-child(5) {
        width: 15%;
    }
    .stuadmin_box tr td:nth-child(6) {
/*         width: 15%; */
    }
    .stuadmin_box tr td:nth-child(7) {
        width: 15%;
    }
    .stuadmin_box .table_topic {
        font-size: 20px;
...
<!-- 노드를 클릭하여 해당내역출력폼 -->
<div id="updateForm" style="height:100%;  overflow-y:scroll;"></div>
<!-- 파일변경 -->
...
<script type="text/javascript">
    ...
    $("#inputFile").on("change", function (e) {
        ...
        const stuCdArr = $(".stuCd_only");
        /* 클릭할 때마다 변하는 학번 중 제일 마지막 학번에 해당하는 파일 내용을 변경하기 위함 */
        const stuCd = $(stuCdArr[stuCdArr.length - 1]).val();
        formData.append("userCd", stuCd);

        // console.log(formData);

        //사진변경
        $.ajax({
            url: "/employee/stuadmin/upload",
            type: "post",
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).on("click", ".submit_btn", function (event) {
        ...
        const data = {
            ...
            //userVO안에 studentVO를 보낼때에는 이런식으로 표현
            studentVO: {
        ...
        $.ajax({
            ...
            success: function (res) {
                //update한 내용(userVO, studentVO) 모두 업데이트완료시 해당 내용으로 모달창내역 변경
                if (res == 2) {
                    ...
                    $("#comdCd").val(data.studentVO.comdCd);
                    $("#selectDept").val(data.studentVO.depCd);
                    $("#stuYear").val(data.studentVO.stuYear);
                    $("#stuSem").val(data.studentVO.stuSem);
                    $("#stuAddt").val(data.studentVO.stuAddt);
                    $("#stuGrdt").val(data.studentVO.stuGrdt);
                ...
                success: (res) => {
                    ...
                    $.each(res, function (idx, chart) {
                        ...
                        let tempObj = {
                            ...
                            // text에 이름을 저장
                            text: chart.organizationName + "(" + chart.organizationDescription + ")",
                            // 노트의 유형을 저장하는 부분
                ...
                success: res => {
                    ...
                    let str = `
                                <div class="stuadmin_box">
			                    	...
			                    	<span style="font-size:12px; color:#888888;"><a href="/employee/stuadmin/stuinfo">학생관리</a></span>
			                    	<span style="font-size:12px; color:#888888;">></span>
			                    	<span style="font-size:12px; color:#888888;"><a href="/employee/stuadmin/stuinfo">학생목록</a></span>
			                    	</div>
                    				<div class="table_topic" style="margin-top:20px;">학생목록<i class="fa-solid fa-circle-info" style="color: #333333;"></i></div>
                                    <table>
                    ...
                    res.forEach(student => {
                        ...
                        //file유무를 fileYn에 담음
                        let fileYn = "";
                        if (student.filesDetailVO == null) fileYn = `<td></td>`
                        else fileYn = `<td>O</td>`
                        ...
                        str += `
                                        <tr>
                                            ...
                                            <td><div class="stuModal" data-stu-cd="\${student.stuCd}" style="cursor:pointer;">\${student.stuCd}</div></td>
                                            <td><div class="stuModal" data-stu-cd="\${student.stuCd}" style="cursor:pointer;">\${student.userVO.userNm}</div></td>
                                            \${btnText}
                                            \${fileYn}
                                        </tr>
                            `
                    ...
                    $('.stuModal').on("click", function () {
                        ...
                        const collegeCode = stuCd.departmentVO.colCd;
                        ...
                        str = `				
                            ...
                            if (stuCd.filesDetailVO != null) {
                                str += `<img src="/resources/upload/\${stuCd.filesDetailVO.fileSvnm}" id="uploadFile" class="uploadFiles" style="min-width:145px; max-width:145px; height:180px; object-fit: cover;"/>`
                            } else {
                                            ...
                                            <table>
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input required id="userNm" name="userNm" type="text" value="\${stuCd.userVO.userNm}" readonly/>
                                                    </td>
                                                    <td>
                                                        <div class="th_span">영문이름</div>
                                                        <input id="userNme" name="userNme" type="text" value="\${stuCd.userVO.userNme}" readonly/>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                        <div class="th_span">생년월일</div>
                                                        <input onclick="birth()" name="userReg1" type="text" value="\${stuCd.userVO.userReg1.substring(0,2)}.\${stuCd.userVO.userReg1.substring(2,4)}.\${stuCd.userVO.userReg1.substring(4,6)}" readonly disabled="disabled" style="background-color:#ffffff;"/>
                                                    </td>
                            `
                            /* 성별은 주민등록번호에 의해 자동입력 */
                            if (stuCd.userVO.userReg2.substring(0, 1) == '2') {
                        ...
                        str += `
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="th_span">학번</div>
                                                        <input id="stuCd" name="studentVO.stuCd" type="text" value="\${stuCd.stuCd}" class="stuCd_only" readonly disabled="disabled" style="background-color:#ffffff;"/></td>
                                                    <td>
                                                        <div class="th_span">주민등록번호</div>
                                                        <div style="display:flex; align-items:center;">
                                                            <input id="userReg1" type="text" name="userReg1" value="\${stuCd.userVO.userReg1}" readonly/> <span>-</span> <input id="userReg2" type="text" name="userReg2" value="\${stuCd.userVO.userReg2}" readonly/></td>
                                                        </div>
                                                </tr>
                                            </table>
                                        </div>
                                        ...
                                        <table>
                                            <button type="button" class="btn-two mini dark addr_btn" onclick="stuinfo_execDaumPostcode()" style="display:none; margin-left:0px;">주소검색</button>
                                            <tr>
                                                <td>
                                                    <div class="th_span">우편번호</div>
                                                    <input id="userZip" type="text" name="userZip" value="\${stuCd.userVO.userZip}" class="input_border" readonly/>
                                                    <div class="th_span">주소</div>
                                                    <input id="userAddr1" type="text" name="userAddr1" value="\${stuCd.userVO.userAddr1}" class="input_width" readonly/>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <div class="th_span">상세주소</div>
                                                    <input id="userAddr2" class="userAddr2" type="text" name="userAddr2" value="\${stuCd.userVO.userAddr2}" readonly/>
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <input id="userBank" type="text" name="userBank" value="\${stuCd.userVO.userBank}" class="input_border" readonly/>
                                                    <div class="th_span">계좌번호</div>
                                                    <input id="userAcc" type="text" name="userAcc" value="\${stuCd.userVO.userAcc}" class="input_width" readonly/>
                                                </td> 
                                            </tr>
                                            <tr>
                                                <td>
                                                    <div class="th_span">연락처</div>
                                                    <input id="userTel" name="userTel" type="text" value="\${stuCd.userVO.userTel}" class="input_border" readonly/>
                                                    <div class="th_span">이메일</div>
                                                    <input id="userMail" type="text" name="userMail" value="\${stuCd.userVO.userMail}" class="input_width" readonly/>
                                                </td>
                                            </tr>
                                        </table>
                                        ...
                                        <table>
                                            <tr>
                                                <td>
                                                    <div class="th_span">학적상태</div> 
                                                    <select id="comdCd" name="comdCd" readonly style="width: 395px;">
                                                        <option value="\${stuCd.commonDetailVO.comdCd}">\${stuCd.commonDetailVO.comdNm}</option>
                                                        <option value="RECD01">재학</option>
                                                        <option value="RECD02">휴학</option>
                                                        <option value="RECD03">자퇴</option>
                                                        <option value="RECD04">재적</option>
                                                        <option value="RECD05">졸업</option>
                                                    </select>
                                                </td>
                                            </tr>
                                            <tr>
                        ...
                        str += `
                            ...
                            `
                            const colCd = stuCd.departmentVO.colCd;
                        ...
                        str += `
                            ...
                            `
                                    if (dept.depCd == stuCd.departmentVO.depCd) {
                        str += `					
                                                        selected
                            `
                                    }
                        ...
                        str += `
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <select id="stuYear" name="studentVO.stuYear" readonly>
                                                        <option class="first_select" value="\${stuCd.stuYear}">\${stuCd.stuYear}</option>
                                                        <option value="1">1</option>
                                                        <option value="2">2</option>
                                                        <option value="3">3</option>
                                                        <option value="4">4</option>
                                                    </select>
                                                    <div class="th_span" style="border-left:1px solid #ebebeb; box-sizing:borderbox;">학기</div> 
                                                    <select id="stuSem" name="studentVO.stuSem" readonly>
                                                        <option class="first_select" value="\${stuCd.stuSem}">\${stuCd.stuSem}</option>
                                                        <option value="1">1</option>
                                                        <option value="2">2</option>
                                                    </select>
                                                </td>
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <input id="stuAddt" name="studentVO.stuAddt" type="date" value='\${stuCd.formAddt}' class="input_border" readonly required/>
                                                    <div class="th_span" style="border-left:1px solid #ebebeb; box-sizing:borderbox;">졸업일자</div> 
                                                    <input id="stuGrdt" name="studentVO.stuGrdt" type="date" value='\${stuCd.formGrdt}' readonly/>
                                                </td>
                                            </tr>
                                        </table>
                                        <div style="position:relative; right:0;">
                                        <button type="button" class="modify_btn btn-two mini dark" style="width:80px; height:25px; padding:5px 10px; margin-top:60px; margin-left:0px;">정보수정</button>
                                        <button type="button" data-stu-cd="\${stuCd.stuCd}" class="submit_btn btn-two mini dark" style="display:none; width:60px; height:25px; padding:5px 10px; margin-top:60px; margin-left:0px;">완료</button>
