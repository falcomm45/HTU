...
.publicize_box tr td:nth-child(1) {
	width: 5%;
}

.publicize_box tr td:nth-child(2) {
	width: 32%;
}

.publicize_box tr td:nth-child(3) {
	width: 11%;
}

.publicize_box tr td:nth-child(4) {
	width: 10%;
}

.publicize_box tr td:nth-child(5) {
	width: 42%;
}
...
<span><a href="/employee/mypage"><svg xmlns="http://www.w3.org/2000/svg" height="13" width="13" viewBox="0 0 576 512" style="margin-right: 5px;"><path fill="#888888" d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z"/></svg></a></span>
	...
	<span style="font-size:12px; color:#888888;">></span>
	<span style="font-size:12px; color:#888888;"><a href="/common/mypage/myBoardList">내가 쓴 게시글</a></span>
	</div>
	<div class="row">
		<div class="col-md-12">
			<div class="card" style="width: 102%;">
				<div class="topic">
					<div class="table_topic">내가 쓴 게시글<i class="fa-solid fa-circle-info" style="color: #333333; position: absolute; top: 2px; left: 130px;"></i></div>

					<!------- 검색기능 ------->
					<form class="searchForm" action="/common/board/mypage/myBoardList"
...
<!-- 											<th width="120px;" class="text-center"><b>작성자</b></th> -->
									...
									<tbody>
										<c:forEach var="boardVO" items="${myBoardVOList}" varStatus="stat">
											<tr role="row" class="odd">
												<td>${boardVO.rnum}</td>
<%-- 												<td class="btnDetail left-aligned-column" data-board-code="${boardVO.comdCd}"> --%>
<%-- 										            <a style="cursor: pointer;" onclick="redirectToDetail('${boardVO.comdCd}', '${boardVO.bdCd}')">${boardVO.bdTitle}</a> --%>
<!-- 										        </td> -->
												<td style="text-align: left;">
									                <c:if test="${boardVO.comdCd eq 'BORD01'}">
									                	<a href="/common/board/free/detail?bdCd=${boardVO.bdCd }" style="cursor: pointer;">${boardVO.bdTitle}</a>
									                </c:if>
									                <c:if test="${boardVO.comdCd eq 'BORD02'}">
									                	<a href="/common/board/publicize/detail?bdCd=${boardVO.bdCd }" style="cursor: pointer;">${boardVO.bdTitle}</a>
									                </c:if>
									                <c:if test="${boardVO.comdCd eq 'BORD03'}">
									                	<a href="/common/board/anonymous/detail?bdCd=${boardVO.bdCd }" style="cursor: pointer;">${boardVO.bdTitle}</a>
									                </c:if>
								                </td>
<%-- 												<td>${boardVO.userVO.userNm}</td> --%>
...
<!-- 								<form id='actionForm' action="/common/mypage/myBoardList" -->
<!-- 									method="get"> -->
...
// 	function redirectToDetail(comdCd, bdCd) {
// 	    let redirectUrl;
	
// 	    if (comdCd === 'BORD01') {
// 	        redirectUrl = '/common/board/free/detail?bdcd=' + bdCd;
// 	    } else if (comdCd === 'BORD02') {
// 	        redirectUrl = '/common/board/publicize/detail?bdcd=' + bdCd;
// 	    } else if (comdCd === 'BORD03') {
// 	        redirectUrl = '/common/board/anonymous/detail?bdcd=' + bdCd;
// 	    } else {
// 	        // 기타 케이스에 대한 처리
// 	        alert('유효하지 않은 comdCd입니다.');
// 	        return;
// 	    }
