...
<div class="counsel_box">
	...
	<div class="table_topic" style="display: flex;">
		<div style="flex: 1;">자료실<i class="fa-solid fa-circle-info" style="color: #333333;"></i></div>
	</div>
	<table>
		<div style="position:relative;"><a href="/professor/lecturedata/create?lecCd=${param.lecCd}" class="btn-reg bur" style="position:absolute; top:-40px; right:0;">등록</a></div>
		...
		<c:forEach var="lectureDataVO" items="${data.content}" varStatus="stat">
		<c:if test="${lectureDataVO.lecCd eq param.lecCd}">
			<tr>
				<td>${stat.count}</td>
				<td style="text-align: left;"><a href="/professor/lecturedata/detail?lecdCd=${lectureDataVO.lecdCd}&lecCd=${lectureDataVO.lecCd}">${lectureDataVO.lecdTitle}</a></td>
...
$(function(){
	...
	if (lecturedataUpdate == 1) {
		alertSuccess('자료 수정완료');
	}
	if (lecturedataCreate == 1) {
		alertSuccess('자료 등록완료');
	}
	if (lecturedataDelete == 1) {
		alertSuccess('자료 삭제완료');
	}
});
</script>